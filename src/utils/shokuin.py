...
def _draw_horizontal_lines(draw, radius, center):
    """横線を描画する"""
    line_y_positions = [center - radius / 3, center + radius / 3]
    ...
    for y_pos in line_y_positions:
        ...
        draw.line(
            [(center - x_length, y_pos), (center + x_length, y_pos)],
            fill="red",
            width=3,
        )
...
def create_shokuin(
    text1, text2, text3, size=200, margin_ratio=0.1, border_width_ratio=0.03
) -> Image:
    """職印画像を生成する"""
