...
def draw_circle(draw, size, radius, center):
    """背景の円を描画する"""
    draw.ellipse(
        [
            center - radius,
...
def draw_text(draw, text, size, radius, center, area="top"):
    """テキストを描画する
...
def draw_horizontal_lines(draw, radius, center):
    """横線を描画する"""
    line_y_positions = [
        center - radius/3,
        center + radius/3
...
def create_shokuin(text1, text2, text3,size=200, margin_ratio=0.1, border_width_ratio=0.03) -> Image:
    ...
    draw_circle(draw, size, radius, center)
    draw_horizontal_lines(draw, radius, center)

    draw_text(draw, text1, size, radius, center, area="top")
    draw_text(draw, text2, size, radius, center, area="center")
    draw_text(draw, text3, size, radius, center, area="bottom")
